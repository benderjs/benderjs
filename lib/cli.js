...
function serverList() {
	forever.list( false, function( err, list ) {
		if ( err || !list || !( ( list = list.filter( function( item ) {
			return ( /\/bin\/bender$/i ).test( item.file );
		} ) ).length ) ) {
			log.info( 'No running Bender servers.' );
			return;
		}
		...
		log.info( chalk.blue( 'uid   running  directory' + new Array( longest - 6 ).join( ' ' ) + 'uptime' ) );
		...
		list.forEach( function( item ) {
			...
			log.info( '%s  %s  %s  %s', item.uid, item.running, chalk.grey( item.cwd ), chalk.yellow( uptime( item.ctime ) ) );
		} );
	} );
}
...
function startDaemon() {
	...
	child.on( 'exit', function() {
		log.error( 'child process stopped' );
	} );
}
