...
module.exports = {
	...
	attach: function( options ) {
		...
		function load( configPath ) {
			var projectConfig = new Config( configPath ).parse( true ),
				...
				localConfig = new Config(
					path.resolve( process.cwd(), '.bender/' + constants.CONFIG_NAME )
				).parse();
		...
		/**
		 * Check if basePath is needed to build tags
		 * @param  {Object}  config Bender config
		 * @return {Boolean}
		 */
		function isBasePathNeeded( config ) {
			var basePaths = [];
		...
		this.conf.needsBasePath = isBasePathNeeded( this.conf );
	}
};
