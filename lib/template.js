...
module.exports = {
	...
	attach: function() {
		...
		template.replaceTags = function( html, data ) {
			var tags = {
				// base path to applications
				APPS_DIR: data.jobId ? '/jobs/' + data.jobId + '/apps/' : '/apps/',
		...
		template.build = function( data ) {
			data.addCSS = function( url, containerTag ) {
				data.addFile( '<link rel="stylesheet" href="' + url + '">', containerTag );
			};

			data.addJS = function( url, containerTag ) {
				data.addFile( '<script src="' + url + '"></script>', containerTag );
			};

			data.addFile = function( include, containerTag ) {
				containerTag = containerTag ? containerTag : 'head';
				data.parts.push( '<' + containerTag + '>' + include + '</' + containerTag + '>' );
			};
