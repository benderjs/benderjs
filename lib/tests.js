...
module.exports = {
	...
	attach: function() {
		...
		/**
		 * Build tests for a group
		 * @param  {String}         name  Name of a group
		 * @param  {Array.<Object>} files Array of files found in a group's base path
		 * @return {Promise}
		 * @memberOf module:tests
		 * @method buildGroup
		 */
		tests.buildGroup = function( name, files ) {
			...
			group.files.forEach( function( file ) {
				stats[ file.path ] = Number( file.stats.mtime );
			} );

			if ( !tests.statsCache[ group.name ] || !_.isEqual( stats, tests.statsCache[ group.name ] ) ) {
		...
		tests.get = function( id ) {
			...
			// file is not located in any of the base paths
			...
			function handleStat( stats ) {
				...
				if ( tests.statsCache[ groupName ] &&
					tests.statsCache[ groupName ][ file ] === Number( stats.mtime ) &&
		...
		/**
		 * Get a list of basePaths for the given test group name
		...
		// run all the builders for the given file list object
		function runBuilders( filelist ) {
			return pipeline( bender.testbuilders.list(), filelist );
		}

		// prepare a file list object consumed by test builders
		...
		// create a final group tests array
