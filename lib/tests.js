...
module.exports = {
    ...
    attach: function () {
        ...
        if (!bender.conf || !bender.applications || !bender.assertions) {
            logger.error('Tests module requires: conf, applications, assertions');
            process.exit(1);
        }

        bender.tests = new TestHandler(bender.conf, bender.applications, bender.assertions);
    }
};
