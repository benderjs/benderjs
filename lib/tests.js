	...
	attach: function() {
		...
		function prepareFileList( files, name ) {
			...
			function isIncluded( file ) {
				return includes.some( function( pattern ) {
		...
		tests.get = function( id ) {
			...
			function getGroupPaths( group ) {
				return group.paths.filter( function( pth ) {
					return pth.charAt( 0 ) !== '!';
				} );
			}
			...
			function isExcluded( group, file ) {
				return group.paths.some( function( pth ) {
					return pth.charAt( 0 ) === '!' && minimatch( file, pth.substr( 1 ) );
				} );
			}
			...
			groupName = _.findKey( bender.conf.tests, function( group ) {
				...
				return !isExcluded( group, file ) && getGroupPaths( group ).some( function( groupPath ) {
					return minimatch( filePath, groupPath );
				} );
			} );
