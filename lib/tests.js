		...
		_.forOwn( bender.conf.tests, function( group ) {
			...
			group.paths.forEach( function( result ) {
				...
				if ( result.charAt( 0 ) === '!' ) {
					...
					group.include.push( path.join( group.basePath, result ) );
					group.basePaths.push( path.join( group.basePath, result.split( '/' )[ 0 ] + '/' ) );
				}
			} );
		} );
