...
module.exports = {
	...
	attach: function() {
		...
		bender.checkDeps( module.exports.name, 'testbuilders', 'conf' );
		...
		tests.buildGroup = function( group ) {
			...
			function prepareFileList( results ) {
				console.timeEnd( 'paths ' + group.name );
			...
			function checkStats( group ) {
				...
				return whenAll( stats )
					.then( function( stats ) {
						console.timeEnd( 'stats ' + group.name );
