    ...
    attach: function () {
        ...
        jobs.create = function (data) {
            ...
            function insertTasks(tests) {
                var tasks = tests.map(prepareTask);

                // left intentionally while seeking for doubled-tests issue
                console.log('tests', data.tests);
                console.log('insert tasks', tasks);

                return whenCall(tasksDb.insert.bind(tasksDb), tasks);
            }
        ...
        jobs.fetch = function (client) {
            ...
            // find unresolved within testTimeout
            browserTasksDb
                .findOne({
                    $and: [
                        { status: 1 },
