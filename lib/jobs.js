...
module.exports = {
	...
	attach: function() {
		...
		jobs.create = function( data ) {
			...
			function prepareDir() {
				logger.debug( 'prepare job directory' );
			...
			function prepareTask( task ) {
				logger.debug( 'prepare job tasks' );
			...
			function fillBrowserTasksDb( tasks ) {
				...
				data.browsers.map( function( browserId ) {
					...
					tasks.forEach( function( task ) {
						// TODO improve filtering mechanism

						// don't create manual tasks for jobs that don't support such
			...
			function copyTests( tasks ) {
				...
				var dest = path.join( jobsDir, id, 'tests' ),
					paths = tasks.reduce( function( result, current ) {
						...
						// TODO use 'basePaths' here
						var dir = bender.conf.tests[ current.group ].basePath;

						// add unique basePaths only
		...
		jobs.get = function( id ) {
			...
			return jobs
				...
				.then( function( results ) {
					...
					if ( results[ 0 ] && results[ 0 ].browsers ) {
