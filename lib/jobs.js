...
module.exports = {
    ...
    attach: function () {
                ...
                return when
                    .map(paths, function (dir) {
                        ...
                        return whenCall(bender.utils.mkdirp, to)
                            .then(function () {
                                return whenCall(ncp, dir, to, { followSymlink: true });
                            });
                    })
                    .then(function () {
                        return tasks;
                    ...
                    return when
                        ...
                        .then(function (result) {
                            return result.sort(function (prev, next) {
                                return prev.created < next.created ? 1 :
                                    prev.created > next.created ? -1 : 0;
                            });
                        });
                });
        };
            ...
            function handleBrowserTask(err, result) {
                ...
                function updateTask(err, task) {
                    ...
                    browserTasksDb.update(
                        { _id: result._id },
                        { $set: { status: jobs.STATUS.PENDING } },
                        run
                    );
                }
                            ...
                            $or: [
                                // failed waiting for retry
                                {
                                    $and: [
                                        ...
                                        { retries: { $lt: bender.conf.testRetries + 1 } }
                                    ]
                                },
                                // unresolved within testTimeout
                                {
                                    $and: [
                                        ...
                                        { created: { $lt: new Date() - bender.conf.testTimeout } }
                                    ]
                                },
                                // waiting
                                {
        ...
        jobs.completeTask = function (data) {
            var query = {
                $set: {
                    status: jobs.STATUS[data.success ? 'PASSED' : 'FAILED']
                }
            };
