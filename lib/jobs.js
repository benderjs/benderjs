...
module.exports = {
	...
	attach: function() {
		...
		function updateBrowserTasks( id, tasks, newBrowsers ) {
			newBrowsers = newBrowsers.map( prepareBrowser );
			...
			function addBrowserTasks() {
				var browsersTasks = [];

				tasks.forEach( function( task ) {
					newBrowsers.forEach( function( browser ) {
						browsersTasks.push(
							{
								name: browser.name,
			...
			return removeBrowserTasks()
				.then( addBrowserTasks );
		}
			...
			function fillTasksDb() {
				...
				return bender.tests
					...
					.then( function insertTasks( tests ) {
						...
						return whenCall( jobs.db.tasks.insert, tests.map( function( task ) {
							return prepareTask( id, task );
						} ) );
					} );
			}
		...
		/**
		 * Reset jobs tasks and if job does not have a snapshot
		 * recreate its list of tests with provided tests.
		 * @param {String} id Job ID
		 * @param {Array} Array of tests ids
		 * @return {Promise}
		 * @memberOf module:jobs
		 * @method recreate
		*/
				...
				function addTasks() {
					return bender.tests
						...
						.then( function( tests ) {
							...
							return whenCall( jobs.db.tasks.insert, filtered.map( function( task ) {
								return prepareTask( id, task );
							} ) );
						} );
				}
			...
			return jobs
				...
				.then( function( job ) {
					...
					if ( oldJob.snapshot ) {
						...
						promise = updateTasks()
							.then( function( tasks ) {
								return updateBrowserTasks( id, tasks, oldJob.browsers );
							} );
					}
		...
		jobs.edit = function( id, data ) {
			...
			return jobs
				...
				.then( function( job ) {
					...
					var browsersChanged = _.difference( data.browsers, oldJob.browsers ).length !== 0,
						promise = updateJob()
							.then( getTasks );

					if ( browsersChanged ) {
						promise = promise.then( function( tasks ) {
							return updateBrowserTasks( id, tasks, data.browsers );
						} );
					}
