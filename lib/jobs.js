...
module.exports = {
    ...
    attach: function () {
        ...
        jobs.list = function () {
            return whenCall(jobsDb.find.bind(jobsDb), {})
                .then(function (results) {
                    return when.map(results, function (job) {
                        return jobs.getResults(job)
                            .then(jobs.compactResults);
                    });
                });
        };
         ...
         * @return {Promise}
         */
        jobs.getResults = function (job) {
            function prepareBrowser(browser) {
                delete browser.created;
                delete browser._id;
                delete browser.taskId;
            ...
            function prepareTask(task) {
                return whenCall(
                    ...
                    .then(function (browsers) {
                        task.results = browsers.map(prepareBrowser);
            ...
            return whenCall(tasksDb.find.bind(tasksDb), { jobId: job._id })
                .then(function (tasks) {
