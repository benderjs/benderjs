		...
		function PageBuilder( name, options ) {
			...
			this.attach = function() {
				...
				if ( html && ( idx = bender.pagebuilders.indexOf( html.build ) ) > -1 ) {
					bender.pagebuilders.splice( idx, 0, that.build.bind( bender ) );
				} else {
					bender.pagebuilders.push( that.build.bind( bender ) );
				}
			};
		}
			...
			add: function( options ) {
				...
				if ( type === 'reporter' ) {
					...
					if ( Array.isArray( group ) ) {
						group.push( plugin.build || plugin.process );
					} else {
						group[ name ] = plugin;
					}
				}
			...
			load: function() {
				...
				bender.conf.plugins.forEach( function( name ) {
					...
					try {
						...
						logger.error( 'Couldn\'t include plugin: ' + name, err );
						process.exit( 1 );
					}
