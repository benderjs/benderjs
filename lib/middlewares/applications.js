 ...
 * @return {Function}
 * @memberOf module:middlewares/applications
 */
	...
	return function( req, res, next ) {
		...
		if ( match && ( app = bender.applications.findOne( 'url', match[ 1 ] ) ) ) {
			...
			if ( app.proxy ) {
				...
				bender.files.send( decodeURIComponent( path.resolve( app.path, filePath ) ), req, res ).catch( resume );
			}
			// nothing to do here
		} else {
			resume();
...
/**
 * @module middlewares/applications
 */
module.exports = {
