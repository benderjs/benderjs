		...
		function serveJob( job ) {
			...
			function sendAsset() {
				...
				if ( !job.snapshot ) {
					...
					bender.files.send( jobFilePath, req, res, resume, filePath );
				}
			}
			...
			function sendApp() {
				bender.jobs.getApp( job._id, url[ 3 ] )
					.then( function( app ) {
						...
						if ( job.snapshot ) {
							bender.files.send( jobFilePath, req, res, resume, appFilePath );
						} else {
							bender.files.send( appFilePath, req, res, resume );
						}
					} );
			}
