	...
	return function( req, res, next ) {
		...
		function serveJob( job ) {
			...
			function sendFile() {
				...
				if ( job.snapshot ) {
					filePath = path.join( process.cwd(), '.bender/', url.join( '/' ) );

					bender.files.get( bender.utils.stripParams( filePath ) )
						.then( function( file ) {
							if ( !file ) {
								resume();
							}

							file.send( res, resume );
						} );
				} else {
					filePath = path.join( process.cwd(), url.slice( 3 ).join( '/' ) );

					send( req, bender.utils.stripParams( filePath ) )
						.on( 'error', resume )
						.pipe( res );
				}
			}
		...
		if ( req.method === 'GET' ) {
			...
			if ( url[ 1 ] && url[ 2 ] === 'coverage' ) {
				return send(
						req,
						bender.utils.stripParams( path.join( process.cwd(), '.bender/', url.join( '/' ) ) )
					)
					.on( 'error', resume )
					.pipe( res );
			}
