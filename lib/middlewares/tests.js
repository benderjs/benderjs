 ...
 *
 * @file Builds test context and serves test assets
 */
...
/**
 * Create a HTTP handler that servers test contexts and files
 * @return {Function}
 * @memberOf module:middlewares/tests
 */
function build( bender ) {
	return function( req, res, next ) {
		...
		// exit on other request types than GET
		if ( req.method !== 'GET' ) {
			return resume();
		}
		...
		// redirect to the dashboard to list all tests in the given path
		...
		// add a search string to the test ID (used by i.e. jQuery plugin)
		if ( parsed.search ) {
			id += parsed.search;
		}
...
module.exports = {
	...
	init: function( done ) {
		...
		// replace * and ** with their RegExp representations
