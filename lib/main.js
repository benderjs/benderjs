 ...
 *
 */
function start( options, config, callback ) {
	if ( options.debug ) {
		logger.setDebug( true );
		Error.stackTraceLimit = 1000;
	}

	var bender = getApplication( options, config, callback );
...
/**
 * @param {String[]} [modules] Modules list, if skipped, all modules will be included.
 * @param {Boolean} [removeAddrFile=true]
 * @returns {broadway.App}
 */
function getApplication( options, config, callback, modules, removeAddrFile ) {
	var bender = new broadway.App( {
			delimeter: ':'
		} );
	...
	removeAddrFile = removeAddrFile === undefined ? true : Boolean( removeAddrFile );
	...
	bender.cli = cli.getSubcommander( bender );
	bender.argv = options;
