...
function runTests( options ) {
	...
	function initAndRunTests( bender ) {
		var testId = options[ 0 ],
			browsers = ( options.browser || bender.conf.startBrowser ).split( ',' ),
			ext;

		bender.browsers.build( browsers );
...
function serverRestart( options ) {
	getMeta( function( meta ) {
		if ( meta ) {
			...
			log.error(
				options[ 0 ] ?
					'Couldn\'t find specified Bender server daemon' :
					'Couldn\'t find Bender server daemon running in this folder'
			);
		}
	}, options[ 0 ] );
}
